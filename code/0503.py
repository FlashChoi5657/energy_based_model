...
parser.add_argument("--gaussian_noise", type=float, default=0.4)
...
parser.add_argument("--lr_langevin_min", type=float, default=0.0001)
...
parser.add_argument("--en_L2_reg", type=float, default=0.1)
...
for i in range(number_epoch):
  ...
  for j, (image, _) in enumerate(iter(dataloader_train)):
    ...
    image       = image.to(device)       # clean
    image_noise = image_noise.to(device) # noisy
    ...
    # prediction = nn.Parameter(image_noise, requires_grad=True)
      
    noise = 0.5 * torch.rand_like(prediction) * np.sqrt(list_lr_langevin[i])
    for _ in range(number_step_langevin):
      # add noise
      prediction.data += noise.data
      prediction.data.clamp_(-1.0, 1.0)
      
      # calculate gradients
      loss = -energy(prediction, prediction)
      grad_x = torch.autograd.grad(loss.mean(), prediction, create_graph=True)[0]
      grad_x.data.clamp_(-0.03, 0.03)
      
      # apply gradients
      # prediction.data -= 0.5 * list_lr_langevin[i] * prediction.grad.data
      prediction.data -= 0.5 * list_lr_langevin[i] * grad_x
      
      prediction.data.clamp_(-1.0, 1.0)
      # grad_x.data.detach_()
      # prediction.grad.zero_()
      
    prediction_update = prediction.detach()
    ...
    pos = energy(image, image)
    neg = energy(prediction_update, prediction_update)
    print(neg.shape, pos.shape)
    loss_energy = torch.mean(neg) - torch.mean(pos) + args.en_L2_reg * torch.mean(pos**2 + neg**2)
    ...
    # -------------------------------------------------------------------        
    val_loss_energy_model.append(loss_energy.mean().item())
...
plt.tight_layout()
fig.savefig(f'{date_figure}/last_result{time_1}.png', bbox_inches='tight', dpi=600)
plt.close(fig)