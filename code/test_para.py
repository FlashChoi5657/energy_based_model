...
from utils import permute, to_numpy, init_weights, Self_Energy_log, get_dataloaders, SGLD
...
parser.add_argument("--name", type=str, default='multi-gpu')
...
parser.add_argument("--save_plot", type=int, default=50)
...
def main():
  ...
  if ngpu > 1:    
    ...
    dl_train, dl_test, _, vis_tr = get_dataloaders(dir_data, args.dataset, args.img_size, 
                                                  args.in_channel, args.batch_size, 
                                                  args.train_size, args.train_ratio, args.test_size, 
                                                  args.use_subset, args.use_label, scale_range, 
                                                  args.use_unpaired, num_workers=args.num_worker)
  ...
  train(energy, args, optim_energy, sched_optim_energy, dl_train, dl_test, device, ngpu)
...
def train(energy, args, optim_energy, sched_optim_energy, dataloader_train, dataloader_test, device, ngpu):
  # ======================================================================
  # Options
  # ======================================================================
  ...
  for i in range(number_epoch):
    ...
    for j, (image, _) in enumerate(iter(dataloader_train)):
      ...
      prediction = SGLD(update, energy, number_step_langevin, list_lr_langevin[i])
      ...
      value_psnr          = to_numpy(PSNR(image_noise, image))
    ...
    if i % args.save_plot == 0:
      fig, ax = plt.subplots(4, 10, figsize = (3 * 15, 3 * col))
      for k in range(col):
        if ngpu > 1:
          rand = k
        else:
    ...
    if i>100 and val_psnr_langevin_mean[i] < val_psnr_langevin_mean[i-50]:
  ...
  y_update_train    = SGLD(y_train, energy, number_step_langevin, list_lr_langevin[-1])
  ...
  for j, (image_train_, _) in enumerate(iter(dataloader_train)):
    ...
    y_update_train_    = SGLD(y_train_, energy, number_step_langevin, list_lr_langevin[-1])
  ...
  y_update_test    = SGLD(y_test, energy, number_step_langevin, list_lr_langevin[-1])
  ...
  for j, (image_test_, _) in enumerate(iter(dataloader_test)):
    ...
    y_update_test_    = SGLD(y_test_, energy, number_step_langevin, list_lr_langevin[-1])
  ...
  ax[0][1].plot(val_psnr_mean[:fin], color='red', label='ID')
