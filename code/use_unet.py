...
parser.add_argument("--save_plot", type=int, default=40)
...
energy = EnergyModel(in_channel, dim_feature, dim_output, activation='silu', scale_range=scale_range,
...
if NGPU > 1:
    ...
    unet = torch.nn.DataParallel(unet, device_ids=list(range(NGPU)))
...
optim_unet = torch.optim.AdamW(unet.parameters(), lr=lr_energy_model, betas=(args.b1, 0.999))
...
for i in range(number_epoch):
  ...
  if i % args.save_plot == 0:
    ...
    plt.tight_layout()
    fig.savefig(f'{date_figure}/unet_log_epoch:{i}_{time_}.png', bbox_inches='tight', dpi=300)
    plt.close(fig)
...
ax[0][0].plot(val_loss_unet_mean, color='red', label='Loss')
ax[0][0].legend()
...
ax[0][2].plot(val_psnr_mean, color='red', label='ID')
...
plt.tight_layout()
fig.savefig(f'{date_figure}/last_unet_{time_1}.png', bbox_inches='tight', dpi=600)
plt.close(fig)