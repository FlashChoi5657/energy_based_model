...
from utils import UNet_Energy_log, to_numpy, init_weights, get_dataloaders
...
parser.add_argument("--name", type=str, default='use_buffer')
...
for i in range(number_epoch):
  ...
  for j, (image, _) in enumerate(iter(dataloader_train)):
    ...
    # if len(buffer) < 1:
    #   buffer.push(image, image_noise)
    # image, image_noise = buffer.get(image.shape[0])
    ...
    optim_energy.step()
    # buffer.push(image, prediction_update)
