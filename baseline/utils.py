...
class SampleBuffer:
  ...
  def get(self, n_samples, device='cuda'):
    image = random.choices(self.buffer, k=n_samples)
...
def SGLD(x, model, step, lr_rate, noise_decay=1.0):
  x.requires_grad = True
  
  for _ in range(step):
    noise =  noise_decay * np.sqrt(lr_rate) * torch.randn_like(x)
    # noise_decay = 0.9 * noise_decay
    ...
    x.data = x.data - 0.5 * lr_rate * x.grad.data + noise
...
def SGLD_(x, y, model, step, lr_rate, noise_decay=1.0):
  x.requires_grad = True
  
  for _ in range(step):
    noise = torch.sqrt(lr_rate) * torch.randn_like(x)
...
def get_dataloaders(dir_data: str, dir_dataset: str, im_size: int, 
                    ...
                    test_ratio: float = 0.2, use_subset: bool = False,
                    use_label: int = 4, use_unpaired: bool = False, 
...
def Self_Energy_log(number_step_langevin: int) -> str:
    log_pattern = '[%4d/%4d]  '
    log_pattern += '[ loss_ae=%.5f | '
    log_pattern += 'loss_ebm=%.5f | '
    log_pattern += 'LR_ebm=%.7f | '
    log_pattern += 'Clean=%.5f | '
    # log_pattern += 'Noisy=%.5f | '
    log_pattern += f'PSNR_Y:{number_step_langevin}=%.5f | '
    log_pattern += 'time=%.1f ]'
    return log_pattern