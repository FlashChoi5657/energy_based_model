...
from utils import UNet_Energy_log, Self_Energy_log, Custom_Dataset, SGLD, SGLD_sum
...
parser.add_argument("--name", type=str, default='loss_sum')
...
energy = EnResnet(in_channel, dim_feature, activation='swish')
...
for i in range(number_epoch):
  ...
  for j, (image, _) in enumerate(iter(dataloader_train)):
    ...
    # Yn generation
    # -------------------------------------------------------------------
    prediction_update = SGLD_sum(image_noise.detach(), energy, number_step_langevin, list_lr_langevin[i])
...
image_noise_test = image_test + noise * torch.randn_like(image_test)
