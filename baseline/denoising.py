...
parser.add_argument("--dim_feature", type=int, default=48)

# parser.add_argument("--lr_unet", type=float, default=0.0005)
parser.add_argument("--lr_energy_model", type=float, default=0.0001)
parser.add_argument("--lr_langevin_max", type=float, default=0.00004)
parser.add_argument("--lr_langevin_min", type=float, default=0.000002)
parser.add_argument("--iter_langevin", type=int, default=10)
parser.add_argument("--multi_noise", type=bool, default=True)
parser.add_argument("--noise_max", type=float, default=0.85)
...
parser.add_argument("--energy_L2_reg_weight", type=float, default=1.)
...
dim_output            = 1
# use_gp                = args.use_gp
list_lr_langevin      = torch.linspace(args.lr_langevin_max, args.lr_langevin_min, steps=number_epoch)
...
dataloader_train, dataloader_test = get_dataloaders(dir_data, args.dataset, args.img_size, 
                                            ...
                                            use_subset = args.subset, use_label=4,
                                            parallel=False)
# ======================================================================
# Evaluation
# ======================================================================
...
# energy = EnResnet(in_channel, dim_feature, in_channel, activation='swish')
energy = Resneten()
AE = autoencoders()
...
# z_vec = torch.randn(batch, in_channel, args.img_size, args.img_size, device=device)
...
optim_ae = torch.optim.Adam(AE.parameters(), lr=0.0001, betas=(args.b1, 0.999))
...
# ======================================================================
# ExponentialLR
sched_optim_energy = torch.optim.lr_scheduler.StepLR(optim_energy, step_size=10, gamma=sched_gamma)
...
val_loss_ae_model     = np.zeros(number_epoch)
...
for i in range(number_epoch):
  ...
  for j, (image, _) in enumerate(iter(dataloader_train)):
    ...
    # if len(buffer) < 1:
    #   buffer.push(z_vec)
    #   noisy = z_vec.clone().detach()
    # else:
    #   noisy = buffer.get(batch)
    
    prediction = AE(image_noise)
    neg_img = SGLD_(prediction, energy, n_step_LD, list_lr_langevin[i])
    loss_ae = criterion(prediction, neg_img)
    ...
    neg = energy(neg_img.detach())
    pos = energy(image)
    reg_loss = L2_w * (pos**2 + neg**2).mean()
    loss_energy = neg.mean() - pos.mean() 
    loss_energy = loss_energy + reg_loss
    ...
    # buffer.push(prediction_update)
    # -------------------------------------------------------------------
    # Save results for each batch iteration
    # -------------------------------------------------------------------
    val_loss_ae.append(loss_ae.item())
    val_loss_en.append(loss_energy.item())

    value_psnr_ori      = to_numpy(PSNR(prediction, image))
    value_psnr_langevin = to_numpy(PSNR(neg_img, image))
  ...
  # -------------------------------------------------------------------
  val_loss_ae_model[i]     = np.mean(val_loss_ae)
  ...
  log = Self_Energy_log(n_step_LD) % (i, 
                                          number_epoch, 
                                          val_loss_ae_model[i],
  ...
  if i % args.save_plot == 0:
    ...
    for k in range(col):
      ...
      ax[2][k].set_title('Train (AE)')
      ax[2][k].imshow(torchvision.utils.make_grid(prediction[rand].detach().cpu(), normalize=True).permute(1,2,0))
      ax[3][k].set_title('Train (Energy)')
      ax[3][k].imshow(torchvision.utils.make_grid(neg_img[rand].detach().cpu(), normalize=True).permute(1,2,0))
  ...
  # if i> args.terminate and val_psnr_langevin[i] < val_psnr_langevin[i-10]:
...
y_update_train    = SGLD(image_noise_train.detach(), energy, n_step_LD, list_lr_langevin[-1])
...
for j, (image_train_, _) in enumerate(iter(dataloader_train)):
  ...
  y_update_train_    = SGLD(image_noise_train_.detach(), energy, n_step_LD, list_lr_langevin[-1])
...
y_update_test    = SGLD(image_noise_test.detach(), energy, n_step_LD, list_lr_langevin[-1])
...
for j, (image_test_, _) in enumerate(iter(dataloader_test)):
  ...
  y_update_test_    = SGLD(image_noise_test_.detach(), energy, n_step_LD, list_lr_langevin[-1])
...
with open(file_option, 'w') as f:
  ...
  f.write('{}: {}\n'.format('number step langevin', n_step_LD))
